...
import { ShoppingCart, Menu, X, LayoutDashboard, LogOut, QrCode } from 'lucide-react';
        ...
        <nav className="hidden md:flex items-center gap-8">
          ...
          <div className="flex items-center gap-4">
            ...
            <Link
              ...
              title="Scan QR"
            >
              <QrCode className="w-6 h-6 text-[#5DBB63]" />
            </Link>
          ...
          {!isAuthenticated ? (
            <Link to="/login" className="text-sm font-medium text-gray-500 hover:text-[#5DBB63] transition-colors">
              Admin Login
            </Link>
          ) : (
            <div className="flex items-center gap-4">
      ...
      {isMenuOpen && (
        <div className="md:hidden bg-white border-t border-gray-100 p-4 flex flex-col gap-4 shadow-lg absolute w-full">
          ...
          <Link to="/cart" onClick={() => setIsMenuOpen(false)} className="text-gray-800 font-semibold p-2 hover:bg-gray-50 rounded-lg flex items-center gap-2">
            Keranjang 
            <span className="bg-[#FFA726] text-white text-xs px-2 py-1 rounded-full">
              {cartCount}
            </span>
          </Link>
          ...
          <Link
            ...
            className="text-gray-800 font-semibold p-2 hover:bg-gray-50 rounded-lg flex items-center gap-2"
          >
            <QrCode className="w-5 h-5 text-[#5DBB63]" />
            Scan QR
          </Link>
          ...
          {!isAuthenticated ? (
            <Link to="/login" onClick={() => setIsMenuOpen(false)} className="text-sm font-medium text-gray-500 p-2">
              Admin Login
            </Link>
          ) : (
